...
The pack ships six icon generations (iOS 14 through 18 and iOS 26 Liquid Glass). By
...
> **Note:** iOS 14 through 17 and iOS 26 LG icons are also visible in the Blueprint
...
2. `app/src/main/assets/appfilter.xml` (byte-identical because Blueprint reads both)
...
Maintainers run the same local checks before merging; missing or drifted entries
...
Contributor release builds use the committed development keystore as a local
sanity check. Do not publish those APKs.

Official publish builds fail closed unless all signing inputs come from the
maintainer environment and the APK signer certificate matches the documented
official SHA-256 fingerprint:

```bash
IOSICONS_PUBLISH_RELEASE=1 \
IOSICONS_KEYSTORE_PATH=/path/to/release.jks \
IOSICONS_STORE_PASSWORD=... \
IOSICONS_KEY_ALIAS=... \
IOSICONS_KEY_PASSWORD=... \
IOSICONS_RELEASE_CERT_SHA256=<official release certificate sha256> \
  ./gradlew assembleRelease

python3 scripts/icontool.py publish-check
```

The publish guard rejects the committed `iosicons.jks` development keystore even
when it is provided through `IOSICONS_KEYSTORE_PATH`.

## Commit style

- One commit per logical change. "Why" in the subject, not "what".
- Don't amend pushed commits. Follow-up fixes land as new commits.
- Don't add automated co-author trailers. The repo policy rejects them.
